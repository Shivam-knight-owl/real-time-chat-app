...
app.get("/contacts",authenticate,async(req:any,res:any)=>{
    try{
        ...
        res.status(200).json({"contacts":contacts?.map((contact:any)=>{return {contactuserId:contact.id,contactName:contact.username}})});
    }
    catch(err){
        console.log(err);
...
app.get("/messages/:contact_username",authenticate,async(req:any,res:any)=>{
    try{
        ...
        res.status(200).json({message:"Messages fetched successfully",messages:messages.map((msg:any)=>{return msg.text})});
    }
    catch(err){
        console.log(err);
